...
class Bosch_2(OneDataset):
    ...
    def _get_sequence_names(self):
        seq_dir = os.path.join(self.data_dir, self.split)
        return [
            name for name in os.listdir(seq_dir)
            if os.path.isdir(os.path.join(seq_dir, name))
        ]
    ...
    def _get_sequence_infos(self):
        ...
        for sequence_name in sequence_names:
            ...
            if not os.path.exists(ini_path):
                raise FileNotFoundError(f"Missing seqinfo.ini in: {sequence_dir}")

            ini = ConfigParser()
            ini.read(ini_path)

            if "Sequence" not in ini:
                raise KeyError(f"Missing [Sequence] section in: {ini_path}")
            ...
            try:
                ...
                raise ValueError(f"seqinfo.ini parse error in {ini_path}: {e}")
    ...
    def _get_image_paths(self):
        ...
        for sequence_name in sequence_names:
            ...
            for fname in all_imgs:
                ...
                image_paths[sequence_name].append(img_path)

            print(f"{sequence_name}: Found {len(image_paths[sequence_name])} image files.")
        return image_paths
